...
import { getOAuthToken } from '../../updater/token.js';
import { fetchOAuthUsage, type FetchOAuthUsageOptions, type OAuthUsage } from './oauth.js';
import type { SubscriptionUsageEntry } from './types.js';
...
export const MAX_OAUTH_ERROR_DETAIL_LENGTH = 2000;

export function getUpdaterOverrides(): Partial<UpdaterDeps> {
...
export function resolveUpdaterDeps(): UpdaterDeps {
...
export function normalizeOAuthErrorDetail(detail: string): string {
...
export function extractOAuthErrorDetail(error: unknown): string | null {
  ...
  return normalizeOAuthErrorDetail(rawDetail.trim());
}

export function normalizeOAuthFetchError(error: unknown): string {
...
export function buildDebugLogOptions(): DebugLogOptions {
  return {
    enabled: true,
...
export function buildFetchOAuthUsageOptions(
  debug: boolean,
  version?: string
): FetchOAuthUsageOptions | undefined {
...
export async function buildSubscriptionUsageEntry(
  debug: boolean,
  ccVersion?: string
): Promise<SubscriptionUsageEntry> {
  const now = Date.now();
 ...
 */
export function isValidSubscriptionUsage(entry: SubscriptionUsageEntry): boolean {
  // Must have no error
